...
MIT + file LICENSE  
c( person( “Pedro”, “Baltazar”, role = c(“ctb”), email =
“<pedrobtz@gmail.com>” ), person( “Jordan”, “Bradford”, role = c(“cre”),
email = “<jrdnbradford@gmail.com>” ), person( “Peter”, “Meissner”, role
= c(“aut”), email = “<retep.meissner@gmail.com>” ), person( “Kun”,
...
Under the hood the
[`rt_request_handler()`](https://docs.ropensci.org/robotstxt/reference/rt_request_handler.md)
function is called within
[`get_robotstxt()`](https://docs.ropensci.org/robotstxt/reference/get_robotstxt.md).
This function takes an {httr} request-response object and a set of event
handlers. Processing the request and the handlers it checks for various
events and states around getting the file and reading in its content. If
an event/state happened the event handlers are passed on to the
[`request_handler_handler()`](https://docs.ropensci.org/robotstxt/reference/request_handler_handler.md)
along for problem resolution and collecting robots.txt file
transformations:
...
itself. Functions like
[`paths_allowed()`](https://docs.ropensci.org/robotstxt/reference/paths_allowed.md)
do have parameters that allow passing along handler rules or handler
functions.
...
For convenience the package also includes a
[`as.list()`](https://rdrr.io/r/base/list.html) method for robots.txt
files.
